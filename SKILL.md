...
1. **开工前冻结方案，让用户批准。** 用户中途打断纠正，已完成的工作就作废了。
2. **派活前我自己产出契约规格，并让规格自包含。** 工人只读四样：工人合同 + **仓库内契约** + 本轮指令 + 上一窗口交接。工人读不到旧系统，所以**契约没写的，工人只能猜**——契约是我的责任，不是工人的。
...
落盘在 `docs/<契约>.md`。**必须在派活之前由我产出**——工人读不到旧系统（见 `templates/AGENTS.md §1`），契约没写的细节工人只能按最小做法猜。
